...
export default function Contact() {
  ...
  const onSubmit = async (data: ContactForm) => {
    ...
    try {
      ...
      if (response.ok) {
        alert("Strategic inquiry transmitted successfully! Our team will respond within 24 hours.");
      } else {
              ...
              <form className="space-y-6 sm:space-y-8 md:space-y-10" onSubmit={handleSubmit(onSubmit)}>
                ...
                >
                  <span className="flex items-center uppercase tracking-widest text-xs sm:text-sm transition-transform group-hover:scale-105">
                    ...
                    {!isSubmitting && <Send className="ml-3 md:ml-4 group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform" size={16} />}
                  </span>
                </button>
              </form>
